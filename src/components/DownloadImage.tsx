...
const DownloadImage: React.FC<Props> = ({ item }) => {
  return (
    <div>
      <Button variant="ghost" onClick={() => downImg(item.img)}>
        <FluentArrowDownload className="inline-block" />
        <span>720P</span>
      </Button>
      <Button variant="ghost" onClick={() => downImg(item.img, 1920, 1080)}>
        <FluentArrowDownload className="inline-block" />
        <span>1080P</span>
      </Button>
      <Button variant="ghost" onClick={() => downImg(item.img, 2560, 1440)}>
        <FluentArrowDownload className="inline-block" />
        <span>2K</span>
      </Button>
      <Button variant="ghost" onClick={() => downImg(item.img, 3840, 2160)}>
        <FluentArrowDownload className="inline-block" />
        <span>4K</span>
      </Button>
    </div>
